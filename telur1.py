...
def main():
    ...
    # insert table image. 
        ...
        tahun = [2016,2017,2019,2021,2022]
        stunting = [27.5,29.6,27.7,24.4,21.6]
        ...
        ax.set_title('Perubahan Prevalensi Stunting di Indonesia (2017-2022)')
        ...
        #caption
        st.write("Gambar 2. Grafik perubahan prevalensi stunting nasional dari tahun 2017-2022.Dari berbagai sumber")
       ...
       # Sort the data
        ...
        df_telurs_provinsi = df.groupby('PROVINSI')[columns_to_agg].agg(['min', 'max', 'mean'])

        # Select the 'mean' level of the MultiIndex columns in df_telurs_provinsi
        mean_values = df_telurs_provinsi.xs('mean', axis=1, level=1)

        # Sum up the mean values across the telur columns for each province
        df_telur_provinsi = mean_values.sum(axis=1)
        ...
        st.write("Gambar 4. Gambaran konsumsi telur per provinsi. Dari 34 provinsi, NTT menjadi provinsi terendah dalam konsumsi telur")
       # st.write("Dari sini, mari kita buka cerita antara Stunting dan Telurnya")
        ...
        df_produksi_telur_sorted = df_produksi_telur.sort_values(by='PROVINSI')
        ...
        ax.bar(x, df_produksi_telur_sorted['2022'], width=bar_width, label='Produksi Telur per Bulan (Ribu Ton)')
        ...
        st.write("Pemberian pelatihan pertanian dengan memanfaat limbah kotoran ternak sebagai pupuk. Ibu dan calon ibu diberi edukasi pentingnya gizi untuk anak.")
        st.write("Peningkatan ketersediaan dan pembaruan data yang dapat diakses oleh masyarakat. Termasuk komitmen menyediakan data yang jujur apa adanya. Hal ini berguna untuk meningkatkan peranan swasta dan akademisi dalam menganalisa program dan kinerja guna menangani masalah stunting bersama-sama.")
    ...
    # About page
        ...
        st.write("Dashboard and Deployment:")
